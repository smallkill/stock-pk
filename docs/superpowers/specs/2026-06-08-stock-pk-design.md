...
一個極簡的「台股多檔 PK」小工具:選 **2~5 檔**股票、日期區間、投資金額,按計算 → 算出每檔的**含息總報酬率**、最終金額、以及每檔相對**基準(預設 0050,可切換)的差額($與%)**,標出贏家,並用一張多線成長曲線圖一眼看出勝負。
...
- **多檔**:股票槽位 2~5 個;預設 2 槽(使用者選股 + 0050 基準),「+ 新增比較股票」最多加到 5,每槽可移除(保留最少 2)。差額以「每檔 vs 可切換基準(預設 0050)」呈現 + 標贏家(最高最終金額);**不**做兩兩全比(5 檔=10 組太雜)。

## 架構

```
前端(Astro on Cloudflare Pages)
  股票槽 2~5 個(每槽:代號/中文名 → 即時候選清單);預設槽1=空、槽2=0050
  「+ 新增比較股票」(上限 5)、每槽可移除(最少 2)
  日期快捷鈕 / 自訂區間(結束預設今天)
  金額(預設 100,000,可調)
  [計算]
        │ GET /api/compare?tickers=2330.TW,0050.TW,…&from=<unix>&to=<unix>(最多 5)
        ▼
Worker(Cloudflare)
  /api/compare:對每檔代理 Yahoo chart API(避 CORS)+ Cloudflare Cache(過去日資料不變)
     回 [{ ticker, name, series, adjStart, adjEnd, firstDate, lastDate }, …]
  /api/visit:訪客埋點(visits 模式)
  /api/stats:回訪客彙整(至少 total uses)
        ▼
前端計算 + 渲染(共同區間見下)
  每檔:投報率 = adjEnd/adjStart − 1;最終金額 = 本金 × adjEnd/adjStart
        成長序列[i] = 本金 × adj[i]/adj[0]
  差額(每檔 vs 基準 base,base 預設 0050 可切換)= final − finalBase;% = (final − finalBase)/finalBase
  贏家 = 最終金額最高者
  → 數字卡片(每檔:投報率% / 最終金額 / vs基準差額$與%;贏家標 👑)
  → 多線成長曲線(最多 5 條,配色 + 圖例)
```
...
| web `StockSlots`(元件 + client TS) | 2~5 個股票輸入槽(代號/中文名 → 即時候選);+新增 / 移除;每槽記住選定 ticker | stocks.json |
| web `compare.ts`(純函式) | 每檔 ROI / 最終金額 / 成長序列;vs 基準差額$與%;贏家;共同區間貼齊 | — |
| web `chart.ts`(純函式 + DOM) | 把最多 5 條成長序列畫成 SVG 多線折線 + 圖例 | — |
...
2. **候選(輸入時)**:使用者在某個股票槽輸入 → 前端過濾 stocks.json:代號以輸入開頭 OR 中文名含輸入字串,取前 ~8 筆顯示;選定後該槽記住其 `code+suffix`(ticker)。
3. **計算(按鈕)**:前端把所有槽(2~5 檔)的 ticker + 區間 unix 起訖 打給 `/api/compare`。Worker 對每檔代理 Yahoo `v8/finance/chart/<ticker>?period1=&period2=&interval=1d&events=div`,各取 `adjclose` 序列(濾掉 null);以 Cloudflare Cache 快取(key=ticker+from+to;過去日資料不可變)。回每檔的序列 + 起訖 adjclose + 股名 + 該股最早/最末可用日。
...
- **公平比較用「共同可用區間」**:PK 必須**所有檔**比**同一段期間**才公平。實際起點 = `max(from 貼齊, 每檔最早日…)`、實際終點 = `min(to 貼齊, 每檔最末日…)`。若因此被調整(例如某檔較晚上市)→ 明確提示「為公平比較,區間調整為 {實際起} ~ {實際終}(因 {股名} 該期間才有資料)」,所有檔都以此共同區間的起點本金為基準、圖以同一起點對齊本金線。
...
- **純函式單元(vitest)**:
  - `compare.ts`:已知 adjStart/adjEnd → 投報率、最終金額;每檔 vs 基準 → 差額$與%;贏家=最終金額最高;成長序列首值=本金、末值=最終金額;共同區間 = max(各最早)~min(各最末)。
...
定期定額、股息再投入開關(adjclose 已含息)、6 檔以上、兩兩全比矩陣、年化/夏普/回撤等進階指標、美股、即時報價、登入、儲存歷史查詢。先把「最多 5 檔單筆 PK + 多線成長圖 + 訪客數」做到漂亮。
